    ...
    fn execute(&self, hits: &[SearchHit], _field_cache: &FieldCache) -> Result<AggregationResult> {
        ...
        match self.order {
            ...
            TermsSortOrder::CountAsc => {
                buckets.sort_by_key(|a| a.doc_count);
            }
            TermsSortOrder::TermAsc => {
                buckets.sort_by_key(|a| a.key.to_string());
            }
            TermsSortOrder::TermDesc => {
                buckets.sort_by_key(|b| std::cmp::Reverse(b.key.to_string()));
            }
        }
        ...
        if missing_count > 0
            && let Some(missing_key) = self.missing.clone()
        {
