...
async fn test_phase3_parallel_delta_processing() {
    ...
    let results = processor.process_indices_parallel(&indices, &parent_snapshot, snapshot_path).await;
...
async fn test_phase3_enhanced_incremental_snapshot() {
    ...
    let result = manager.create_enhanced_incremental_snapshot(
        &repository_name,
        &snapshot_name,
        &indices,
        None,
    ...
    match result {
        Ok(snapshot_result) => {
            ...
            assert!(snapshot_result.snapshot_info.parallel_processing_info.is_some());
        }
        Err(_) => {
            // Expected for test environment without actual data
...
async fn test_phase3_compressed_snapshot_creation() {
    ...
    let result = repo.create_compressed_snapshot(snapshot_name, request, compression_config).await;
    ...
    match result {
        Ok(snapshot_info) => {
            ...
            assert!(snapshot_info.metadata.settings.contains_key("compression_algorithm"));
            assert!(snapshot_info.metadata.settings.contains_key("compression_level"));
        }
        Err(_) => {
            // Expected for test environment without actual data
