...
use crate::performance::{AnalysisReport, Metrics, Profile};
...
/// Available report formats for performance reports
...
impl PerformanceReporter {
    ...
    pub fn generate_report(
        ...
        match self.format {
            ...
            ReportFormat::Csv => Self::generate_csv_report(self, metrics, profiles, analysis),
        }
    }
    ...
    fn generate_json_report(
        ...
        let report = PerformanceReport {
            metrics: if self.include_details {
                Some(metrics.clone())
            } else {
                None
            },
            profiles: if self.include_details {
                Some(profiles.clone())
            } else {
                None
            },
            analysis: analysis.clone(),
            system_metrics: if self.include_system {
                Some(metrics.system.clone())
            } else {
                None
            },
        };
    ...
    fn generate_yaml_report(
        ...
        serde_yaml::to_string(&report)
            .unwrap_or_else(|_| "Error generating YAML report".to_string())
    }
    ...
    fn generate_html_report(
        ...
        html.push_str(&format!(
            "<tr><td>Total Operations</td><td>{}</td></tr>\n",
            profiles.len()
        ));
        html.push_str(&format!(
            "<tr><td>Total Measurements</td><td>{}</td></tr>\n",
            profiles.values().map(|p| p.count).sum::<u64>()
        ));
...
pub struct PerformanceReport {
    /// Optional detailed metrics data
    pub metrics: Option<Metrics>,
    /// Optional detailed profile data
    pub profiles: Option<HashMap<String, Profile>>,
    /// Performance analysis results
    pub analysis: AnalysisReport,
    /// Optional system metrics data
...
impl BenchmarkRunner {
        ...
        F: Fn() -> R,
    {
        ...
        let avg = Duration::from_nanos((total.as_nanos() / u128::from(count)) as u64);
...
/// Benchmark result containing performance measurements
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BenchmarkResult {
    ...
    /// Minimum time across all iterations
    pub min_time: Duration,
    /// Maximum time across all iterations
