...
use lexum_core::index::{AliasManager, IndexAlias};
...
fn test_alias_operations() {
    ...
    let updated_alias = manager.add_indices_to_alias("test-alias", new_indices).unwrap();
    assert_eq!(updated_alias.index_count(), 3);
    
    // Remove some indices
    let remove_indices = vec![IndexName::new("index2")];
    let final_alias = manager.remove_indices_from_alias("test-alias", remove_indices).unwrap();
